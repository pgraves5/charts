...
bk_log_level = 'INFO'
bookie_bin = "/opt/bookkeeper/bin/bookkeeper"
common_bin = "/opt/bookkeeper/bin/common.sh"
log4j_conf = "/opt/bookkeeper/conf/log4j.properties"

# edit bin/common.sh
ls = []
for l in open(common_bin):
    ls.append(l)

file = open(common_bin, 'w')
for l in ls:
    if l.find("bookkeeper.root.logger") != -1:
        continue
    file.write(l)
file.close()
...
for conf_filename in conf_files:
    ...
    for line in lines:
        # specific line edit for log4j conf
        if conf_filename == log4j_conf:
            if line.startswith("bookkeeper.root.logger"):
                line = "bookkeeper.root.logger=" + bk_log_level + ",ROLLINGFILE" + '\n'
            elif line.startswith("bookkeeper.log.dir"):
                line = "bookkeeper.log.dir=/opt/bookkeeper/logs"  + '\n'
            elif line.startswith("#log4j.appender.ROLLINGFILE.MaxFileSize"):
                line = "log4j.appender.ROLLINGFILE.MaxFileSize=10MB" + '\n'
            elif line.startswith("#log4j.appender.ROLLINGFILE.MaxBackupIndex"):
