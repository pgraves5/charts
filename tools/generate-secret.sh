...
openssl req -new -key "${tmpdir}"/server-key.pem -subj "/CN=${servic}.${namespace}.svc" -out "${tmpdir}"/server.csr -config "${tmpdir}"/csr.conf
...
metadata:
  name: ${csrName}
  annotations:
    networkConnectionName: ${csrName}
    networkConnectionNamespace: ${namespace}
spec:
  groups:
...
kubectl create secret generic ${secret} \
        ...
        --from-file=ca.crt="${tmpdir}"/server-cert.pem \
